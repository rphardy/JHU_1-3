rankhospital <- function(state,outcome,num="best"){
  ...
  if (outcome %in% names(outdict)){
    ...
    #check validity of state, print hospital at rank given by num
    if (state %in% names(statedict)){
      selected <- bystate[statedict[state]]
      numdict <- c("best"=1, "worst"=length(selected[[1]][[1]]))
           if (num %in% names(numdict)){
             print(selected[[1]][[1]][[numdict[num]]])
           } else if(num >0 & num < length(selected[[1]][[1]])){
             print(selected[[1]][[1]][[num]])
           } else return(NA)
    }
